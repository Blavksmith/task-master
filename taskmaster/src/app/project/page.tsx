...
import { PlusIcon, Search, Filter, ArrowUpDown } from "lucide-react";
...
useEffect(() => {
  const fetchProjects = async () => {
    ...
    const projectsWithOwners = filteredProjects.map((project) => {
    ...
    setProjects(projectsWithOwners as ProjectWithOwner[]);
    setLoading(false);
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="flex gap-3">
            <Button variant="outline" size="icon" onClick={() => setSortOrder((prev) => (prev === 'asc' ? 'desc' : 'asc'))} className="h-10 w-10">
              <ArrowUpDown className={`h-4 w-4 transition ${sortOrder === 'asc' ? "rotate-180" : ""}`} />
            </Button>
          </div>
        </div>
